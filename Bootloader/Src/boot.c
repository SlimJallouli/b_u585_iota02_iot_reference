...
#define APP_START_ADDRESS                (FLASH_BASE + (FLASH_PAGE_SIZE * RESERVED_BOOT_SECTORS))
...
void Jump_To_Main_Application(void)
{
  ...
  if (*(uint32_t*)APP_START_ADDRESS != 0x200C0000)
  {
...
uint8_t dst_buffer    [FLASH_PAGE_SIZE];
#endif
uint8_t src_buffer    [FLASH_PAGE_SIZE];
...
void copy_hota(void)
{
  ...
  {
    src_address = HOTA_START_ADDRESS + (sector_idx * FLASH_PAGE_SIZE);
    dst_address = APP_START_ADDRESS  + (sector_idx * FLASH_PAGE_SIZE);

    /* Copy source sector (HOTA) to buffer */
    copy_from_flash_to_buffer(src_buffer, src_address, FLASH_PAGE_SIZE);

#if (SWAP_APPLICATION_AND_OTA == 1)
    copy_from_flash_to_buffer(dst_buffer, dst_address, FLASH_PAGE_SIZE);
    ...
    if (!prvWriteToFlash(src_buffer, dst_address,FLASH_PAGE_SIZE))
    {
      return;
    }
    ...
    if (!prvWriteToFlash(dst_buffer, src_address, FLASH_PAGE_SIZE))
    {
      return;
    }
#endif
...
static bool prvEraseSectors(uint32_t Nb_Bank, uint32_t Nb_Sectors, uint32_t StartSector)
{
  ...
  if (HAL_FLASH_Unlock() == HAL_OK)
  {
    ...
    pEraseInit.Banks     = Nb_Bank;
    pEraseInit.NbPages   = Nb_Sectors;
    pEraseInit.Page      = StartSector;
#endif
