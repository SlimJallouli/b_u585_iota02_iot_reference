...
# Guard: this script is AWS single-thing flow only
if ($brokerType -ne "aws") {
    throw "This script supports broker_type 'aws' only. Current value: '$brokerType'."
}

# Guard: validate compatible firmware profiles
$awsSingleCompatibleConfigs = @("MXCHIP_Single", "ST67_T01_Single", "ST67_T02_Single")
if ($configuration -notin $awsSingleCompatibleConfigs) {
    throw "Configuration '$configuration' is not compatible with provision_aws_single.ps1. Allowed values: $($awsSingleCompatibleConfigs -join ', ')"
}
...
try {
    ...
    if ($configuration -eq "ST67_T01_Single") {
        Send-Command "conf set mqtt_security 4"
    }
