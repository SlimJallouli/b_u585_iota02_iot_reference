...
static SemaphoreHandle_t I2C1_MutexHandle;
#endif
...
int32_t BSP_I2C1_Init_OS(void)
{
  ...
  {
    I2C1_MutexHandle = xSemaphoreCreateMutex();
  }
  taskEXIT_CRITICAL();

  xSemaphoreTake(I2C1_MutexHandle, portMAX_DELAY);
  ret = BSP_I2C1_Init();
  xSemaphoreGive(I2C1_MutexHandle);
...
int32_t BSP_I2C1_IsReady_OS(uint16_t DevAddr, uint32_t Trials)
{
  ...
  xSemaphoreTake(I2C1_MutexHandle, portMAX_DELAY);
  ret = BSP_I2C1_IsReady(DevAddr, Trials);
  xSemaphoreGive(I2C1_MutexHandle);
...
int32_t BSP_I2C1_WriteReg_OS(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  int32_t ret = BSP_ERROR_NONE;

  xSemaphoreTake(I2C1_MutexHandle, portMAX_DELAY);
  ret = BSP_I2C1_WriteReg(DevAddr, Reg, pData, Length);
  xSemaphoreGive(I2C1_MutexHandle);
...
/**
  ...
  * @retval BSP status
  */
int32_t  BSP_I2C1_ReadReg_OS(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  int32_t ret = BSP_ERROR_NONE;

  xSemaphoreTake(I2C1_MutexHandle, portMAX_DELAY);
  ret = BSP_I2C1_ReadReg(DevAddr, Reg, pData, Length);
  xSemaphoreGive(I2C1_MutexHandle);
...
/**
  ...
  * @retval BSP statu
  */
int32_t BSP_I2C1_WriteReg16_OS(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  int32_t ret = BSP_ERROR_NONE;
...
/**
  ...
  * @retval BSP status
  */
int32_t  BSP_I2C1_ReadReg16_OS(uint16_t DevAddr, uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  int32_t ret = BSP_ERROR_NONE;
