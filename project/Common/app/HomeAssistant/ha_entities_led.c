   ...
   */
    ...
    for( int i = 0; i < ( int ) ARRAY_SIZE( xLEDs ); i++ )
    {
        ...
        if( pxLed->enabled == pdTRUE )
        {
            ...
            /* Home Assistant will publish these exact strings to command_topic.
             * They must be valid JSON documents, but escaped because they are
             * embedded inside the discovery JSON as a JSON string.
             */
            ( void ) snprintf( payloadOn,
                               sizeof( payloadOn ),
                               "{\\\"ledStatus\\\":{\\\"%s\\\":{\\\"desired\\\":\\\"ON\\\"}}}",
                               pxLed->name );

            ( void ) snprintf( payloadOff,
                               sizeof( payloadOff ),
                               "{\\\"ledStatus\\\":{\\\"%s\\\":{\\\"desired\\\":\\\"OFF\\\"}}}",
                               pxLed->name );
