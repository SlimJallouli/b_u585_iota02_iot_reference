...
/* Includes ------------------------------------------------------------------*/
#include "../W6X_ARCH_T02/st67_lwip.h"
...
int32_t MX_LWIP_Init(void)
{
 ...
 net_if_cb.link_sta_up_fn   = netif_link_sta_up_cb;
 net_if_cb.link_sta_down_fn = netif_link_sta_down_cb;
  ...
  if (netifapi_netif_add(netif[W6X_NET_IF_STA],
		                 NULL,
						 NULL,
						 NULL,
						 NULL,
