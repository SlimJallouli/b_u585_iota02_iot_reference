...
#if MQTT_ENABLED
/* MQTT structure to receive subscribed message from the ST67W6X_NCP Driver */
extern W6X_MQTT_Data_t *pxMQTTRecvData;

extern QueueHandle_t xSubMsgQueue;
#endif

static   W61_WiFi_Connect_Opts_t ConnectOpts = { 0 };

static W6X_App_Cb_t App_Cb;
...
static int32_t APP_Init(void)
{
  ...
  /* Register the application callback to received events from ST67W6X_NCP Driver */
  ...
  /* Initialize the ST67W6X_NCP Driver */
  ret = W6X_Init();

  if (ret != W6X_STATUS_OK)
  {
    LogError("failed to initialize ST67W6X_NCP Driver, %ld", ret);
  }
...
static int32_t APP_WiFi_Init(void)
{
  ...
  if (ret == W6X_STATUS_OK)
  {
    /* Initialize the ST67W6X_NCP Wi-Fi module */
    ret = W6X_WiFi_Init();

    if (ret != W6X_STATUS_OK)
    {
      LogError("failed to initialize ST67W6X_NCP Wi-Fi component, %ld", ret);
    }
  }
  ...
  if (ret == W6X_STATUS_OK)
  {
    /* Initialize the ST67W6X_NCP Network module */
    ret = W6X_Net_Init();

    if (ret != W6X_STATUS_OK)
    {
      LogError("failed to initialize ST67W6X_NCP Net component, %d", ret);
    }
  }
...
static void APP_net_cb(W6X_event_id_t event_id, void *event_args)
{
  W61_Net_CbParamData_t *p_param_app_net_cb;

  switch (event_id)
  {
  case W6X_NET_EVT_SOCK_DATA_ID:
    p_param_app_net_cb = (W61_Net_CbParamData_t*) event_args;
