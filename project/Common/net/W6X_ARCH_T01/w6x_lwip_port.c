...
struct hostent *W6X_Net_gethostbyname(const char *hostname)
{
    ...
    if (W6X_Net_GetHostAddress(hostname, ip_addr) == W6X_STATUS_OK)
    {
    	host_info.h_length = 4;
        host_info.h_addr_list[0] = ip_addr;
        host_info.h_addr_list[1] = NULL;  // Terminate the list
        return &host_info;
    }
    else
    {
...
#define IP_ADDR_STR_LEN    16  // Maximum length for IPv4 string

char *W6X_Net_inet_ntoa(uint32_t ip_addr)
{
    static char ip_str[IP_ADDR_STR_LEN];
